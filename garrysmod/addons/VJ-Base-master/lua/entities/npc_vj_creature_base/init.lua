...
ENT.InvestigateSoundDistance = 1000 -- How far away can the SNPC hear sounds? | This number is timed by the calculated volume of the detectable sound.
...
ENT.PropAP_MaxSize = 100000-- This is a scale number | x > 1  = Larger props | x < 1  = Smaller props | x = 1  == Default base value
	-- ====== Control Variables ====== --
...
"weapon_stunstick",
"weapon_bugbait",
} -- List of items it will randomly pick from | Leave it empty to drop nothing or to make your own dropping code (Using CustomOn...)
						...
						if phys:IsValid() && phys != nil && phys != NULL then
							...
							if self:PushOrAttackPropsCode({IsSingleValue=v,CustomMeleeDistance=AttackDist}) then
								...
								if self.PushProps == true then
									local phys = v:GetPhysicsObject()
									if MyEnemy != nil then
										--local posfor = phys:GetMass() * 0
										--local posup = phys:GetMass() * 0
...
function ENT:VJ_DoSlowPlayer(argent,WalkSpeed,RunSpeed,SlowTime,SoundData,ExtraFeatures,CustomCode)
	...
	if self.HasSounds == true && vSD_PlaySound == true then
		self.CurrentSlowPlayerSound = CreateSound(argent,VJ_PICK(vSD_SoundTable))
