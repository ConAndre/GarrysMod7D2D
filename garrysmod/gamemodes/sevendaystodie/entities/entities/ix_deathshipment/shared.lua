...
if (SERVER) then
	function ENT:Initialize()
		...
		local inventory = self:GetOwner():GetCharacter():GetInventory():GetItems()
		local invTable = {}
	
		for k,v in pairs(inventory) do
			if 	invTable[v.uniqueID] != nil then
				invTable[v.uniqueID] =  invTable[v.uniqueID] + 1 
			else 
				invTable[v.uniqueID] = 1
			end
		end
		self.items = invTable
	end
	...
	function ENT:OnPopulateEntityInfo(container)
		...
		name:SetImportant()
		name:SetText(L("Player Loot"))
		name:SizeToContents()

		
		local description = container:AddRow("description")
		description:SetText(L("This belongs to ".. self:GetOwner():GetName()))
